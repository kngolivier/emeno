    ...
    <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border border-slate-50 overflow-hidden w-full max-w-lg mx-auto animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="p-6 md:p-8 bg-slate-50/30 border-b border-slate-100 flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-primary italic font-display leading-none uppercase">Nouveau Client</h2>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 text-secondary">Base de données EMENO</p>
          </div>
        </div>
        <button onClick={onCancel} className="p-2 bg-white rounded-xl border border-slate-100 text-slate-300 hover:text-rose-500 hover:border-rose-100 transition-all active:scale-90">
          <X size={20} />
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input 
                required
                className={`${inputClass} md:pl-11 ${errors.nom ? "border-rose-100" : ""}`}
                placeholder="Mba"
                value={client.nom}
