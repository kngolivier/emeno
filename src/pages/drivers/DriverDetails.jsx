...
export default function DriverDetails() {
  ...
  if (!driver) return <div className="p-20 text-center font-display italic text-primary dark:text-white/80">Livreur introuvable</div>;
    ...
    <div className="p-4 md:p-0 space-y-6 md:space-y-10 font-sans max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4 md:gap-6">
          <button 
            onClick={() => navigate(-1)} 
            className="p-3 md:p-4 bg-white dark:bg-slate-900 rounded-xl md:rounded-2xl border border-slate-100 dark:border-slate-800 hover:shadow-lg transition-all text-primary dark:text-white active:scale-90"
          >
            <ChevronLeft size={20} strokeWidth={3} />
          </button>
          <div className="min-w-0 py-1"> 
              <h1 className="text-3xl md:text-5xl font-black text-primary dark:text-white font-display italic tracking-tighter leading-[1.1] pr-2 uppercase">
              ...
              <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-[8px] md:text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest italic">ID: {driver._id.slice(-6)}</span>
              </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button 
            ...
            className="flex-1 md:flex-none px-4 md:px-6 py-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-primary dark:text-white rounded-2xl font-black uppercase tracking-widest text-[9px] md:text-[10px] hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all flex items-center justify-center gap-2 shadow-sm"
          >
          ...
          <button 
            ...
            className="flex-1 md:flex-none px-4 md:px-6 py-4 bg-red-500 text-white rounded-2xl font-black uppercase tracking-widest text-[9px] md:text-[10px] shadow-xl shadow-red-200 dark:shadow-none hover:bg-red-600 transition-all flex items-center justify-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-slate-50 dark:border-slate-800 shadow-soft space-y-6 h-full">
          ...
          <div className="space-y-3">
            <a href={`tel:${driver.telephone}`} className="flex items-center justify-between p-4 bg-slate-50/50 dark:bg-slate-800/50 rounded-2xl hover:bg-white dark:hover:bg-slate-800 hover:shadow-md transition-all group border border-transparent hover:border-slate-100 dark:hover:border-slate-700">
                <div className="flex items-center gap-4">
                    <div className="p-2 bg-white dark:bg-slate-900 rounded-xl shadow-sm text-secondary group-hover:scale-110 transition-transform"><Phone size={18} /></div>
                    <span className="font-bold text-slate-700 dark:text-slate-200 text-sm md:text-base">{driver.telephone}</span>
                </div>
                <ExternalLink size={14} className="text-slate-300 dark:text-slate-600" />
            </a>
            
            <div className="flex items-center gap-4 p-4 bg-slate-50/50 dark:bg-slate-800/50 rounded-2xl border border-transparent">
              <div className="p-2 bg-white dark:bg-slate-900 rounded-xl shadow-sm text-secondary"><Mail size={18} /></div>
              <span className="font-bold text-slate-700 dark:text-slate-200 text-sm md:text-base break-all">{driver.email || "Non renseigné"}</span>
            </div>
          </div>

          <div className="pt-4 border-t border-dashed border-slate-100 dark:border-slate-800">
             <p className="text-[9px] font-black text-slate-300 dark:text-slate-500 uppercase mb-2 ml-1">Adresse de résidence</p>
             <p className="text-sm font-bold text-primary dark:text-white italic bg-slate-50/50 dark:bg-slate-800/50 p-4 rounded-2xl">{driver.adresse || "Libreville, Gabon"}</p>
          </div>
        </div>
          ...
          {[
            ...
            <div key={i} className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-slate-50 dark:border-slate-800 shadow-soft flex flex-row sm:flex-col items-center justify-between sm:justify-center text-left sm:text-center gap-4">
              ...
              <div>
                ...
                <p className="font-display font-black text-2xl md:text-4xl text-primary dark:text-white italic leading-none">{item.val}</p>
              </div>
            </div>
          ))}

          {/* JOIN DATE CARD */}
          <div className="sm:col-span-3 bg-primary dark:bg-white/[0.03] text-white p-6 rounded-[2rem] flex items-center justify-between overflow-hidden relative group border border-transparent dark:border-white/5 shadow-2xl dark:shadow-none">
              ...
              <Clock className="absolute -right-4 -bottom-4 w-24 h-24 opacity-10 dark:opacity-5 group-hover:rotate-12 transition-transform duration-700" />
          </div>
        </div>
      </div>
      ...
      {/* DELETE MODAL */}
      {showDeleteModal && (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center bg-primary/40 dark:bg-black/80 backdrop-blur-md p-0 md:p-4">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-t-[2.5rem] md:rounded-[2.5rem] w-full max-w-sm text-center space-y-6 shadow-2xl border border-transparent dark:border-slate-800 animate-in slide-in-from-bottom md:zoom-in-95">
            <div className="w-16 h-1 w-12 bg-slate-100 dark:bg-slate-800 rounded-full mx-auto md:hidden -mt-4 mb-4" />
            <div className="w-20 h-20 bg-red-50 dark:bg-red-500/10 text-red-500 rounded-3xl flex items-center justify-center mx-auto rotate-3">
              <Trash2 size={32} />
            </div>
            <div>
              <h2 className="font-display font-black text-2xl italic text-primary dark:text-white uppercase">Supprimer ?</h2>
              <p className="text-slate-400 dark:text-slate-500 text-xs md:text-sm mt-2 px-4">Cette action est irréversible. Toutes les données du livreur seront effacées.</p>
            </div>
            <div className="flex flex-col md:flex-row gap-3 pt-2">
              ...
              <button 
                ...
                className="w-full py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-600 hover:text-primary dark:hover:text-white transition-colors"
              >
                Annuler
              </button>
            </div>
