...
export default function ClientCreateOrder() {
  ...
  const isWhatsAppService = selectedService?.pricingMode === 'WHATSAPP_ONLY';
  ...
  const WhatsAppRedirectBlock = ({ service }) => (
    <div className="p-8 border-2 border-dashed border-secondary/30 rounded-[2.5rem] bg-secondary/5 text-center space-y-4 animate-in zoom-in-95">
      ...
      <div>
        <h3 className="font-black text-primary dark:text-white uppercase italic">Service {service.title}</h3>
        <p className="text-xs text-slate-500 mt-2">Ce service nécessite une confirmation via WhatsApp.</p>
      </div>
      <a 
        href={`https://wa.me/${settings.contact.whatsappNumber}?text=Bonjour, je souhaite commander : ${service.title}`} 
        target="_blank" 
        rel="noopener noreferrer"
        className="inline-block bg-green-600 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-green-700 transition-all"
      >
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-24">
      ...
      {/* ================= SERVICE SELECT ================= */}
            ...
            {selectedService ? (
              isWhatsAppService ? (
                // Affichage du bloc WhatsApp si type whatsapp_only
                <div className="pt-8">
                  <WhatsAppRedirectBlock service={selectedService} />
                </div>
              ):
                ...
                {step === 3 && (
                  ...
                  <div className="text-center py-20 text-slate-400 text-xs uppercase font-black italic">
